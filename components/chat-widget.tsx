    ...
    <>
      ...
      {open && (
        <div className="fixed bottom-24 right-4 z-50 flex w-[360px] max-w-[calc(100vw-2rem)] flex-col rounded-2xl border border-border bg-background shadow-2xl">
          {/* Header */}
          <div className="flex items-center gap-3 rounded-t-2xl bg-gradient-to-r from-pink-600 to-fuchsia-500 px-4 py-3">
              ...
              return (
                ...
                >
                  {message.role === "assistant" && (
                    <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-pink-100 dark:bg-pink-900/30 mt-0.5">
                      <Bot className="h-3.5 w-3.5 text-pink-600 dark:text-pink-400" />
                    </div>
                  )}
                  <div
          ...
          <div className="flex gap-2 border-t border-border p-3">
            ...
            <Button
              size="icon"
              className="h-9 w-9 shrink-0 bg-pink-600 hover:bg-pink-700"
              onClick={handleSend}
      ...
      <button
        onClick={() => setOpen((v) => !v)}
        className="fixed bottom-4 right-4 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-gradient-to-br from-pink-600 to-fuchsia-500 shadow-lg shadow-pink-500/25 hover:shadow-xl hover:scale-105 transition-all"
